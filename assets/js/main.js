...
filtered = applyDashboardSizeFilter(filtered);
...
// Setup clickable headers for Vehicle Summary table
setupVehicleSummaryHeaders();
...
// Setup Vehicle Summary table clickable headers
function setupVehicleSummaryHeaders() {
    const sizeFilterBtns = document.querySelectorAll('#dashboard-vehicle-summary-table .size-filter-btn');
    
    sizeFilterBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            const sizeFilter = this.dataset.size;
            filterDashboardVehiclesBySize(sizeFilter);
        });
    });
}

// Filter dashboard vehicles by size
async function filterDashboardVehiclesBySize(sizeFilter) {
    ...
    try {
        ...
        const { data: vehicles, error } = await supabaseClient
            .from('vehicle_registry')
            .select('plate, model, vehicle_type, year_model, no_of_years, size');
        ...
        const filteredVehicles = vehicles.filter(vehicle => {
            ...
            return size.includes(sizeFilter.toLowerCase());
        });
            ...
            dashboardBody.innerHTML = `
                <tr>
                    <td colspan="5" style="text-align: center; padding: 20px; color: #666;">
                        No vehicles found for ${sizeFilter}
                    </td>
                </tr>
            `;
        }
...
<td>${client.total_km ? (Number.isInteger(parseFloat(client.total_km)) ? parseFloat(client.total_km).toString() : parseFloat(client.total_km).toFixed(2)) : '-'}</td>
...
totalKm.value = Number.isInteger(total) ? total.toString() : total.toFixed(2);
...
// Global variable to track dashboard size filter
window.currentDashboardSizeFilter = null;

// Setup size filter buttons for Dashboard Vehicle Summary table
function setupDashboardSizeFilterButtons() {
  const sizeFilterBtns = document.querySelectorAll('.summary-filter-btn');
  sizeFilterBtns.forEach(btn => {
    btn.addEventListener('click', function() {
      const size = this.getAttribute('data-size');
      handleDashboardSizeFilterClick(size, this);
    });
  });
}

// Handle dashboard size filter button click
function handleDashboardSizeFilterClick(size, buttonElement) {
  console.log('Dashboard size filter clicked:', size);
  
  // Toggle filter: if clicking the same size, clear the filter
  if (window.currentDashboardSizeFilter === size) {
    window.currentDashboardSizeFilter = null;
    ...
    console.log('Dashboard size filter cleared');
  } else {
    window.currentDashboardSizeFilter = size;
    // Remove highlight from all buttons
    document.querySelectorAll('.summary-filter-btn').forEach(btn => {
      btn.style.backgroundColor = '';
      btn.style.color = '';
    ...
    console.log('Dashboard size filter set to:', size);
  }
  
  // Reload dashboard vehicle data with the filter
  loadDashboardVehicleSummary(selectedPlantFilter);
}

// Apply dashboard size filter to vehicle data
function applyDashboardSizeFilter(vehicles) {
  if (!window.currentDashboardSizeFilter) {
    return vehicles;
  }
  
  const filterSize = window.currentDashboardSizeFilter.toLowerCase().trim();
  console.log('Applying dashboard size filter:', filterSize);
  
  const filtered = vehicles.filter(vehicle => {
    const size = (vehicle.size || '').toLowerCase();
    const vehicleType = (vehicle.vehicle_type || '').toLowerCase();
    return size.includes(filterSize) || vehicleType.includes(filterSize);
  });
  
  console.log('Filtered to ' + filtered.length + ' vehicles with size: ' + window.currentDashboardSizeFilter);
  return filtered;
}

// Initialize dashboard size filter buttons
setupDashboardSizeFilterButtons();
